...
#     "_source": [ "path", "method"]
# }
# respDoc=client.search(index=index_name,body=query)
# respDoc=client.get(index=index_name)
# for doc in respDoc["hits"]["hits"]:
#     print(doc["_source"])
...
for path, methods in spec.get("paths", {}).items():
    ...
    for method, details in methods.items():
        ...
        # semantic_text=semantic_response
        embedding = sentencemodel.encode(semantic_text).tolist()
        print("###################")
        print(len(embedding))
        if embedding is None or len(embedding) != 384:
            print("######################")
            print("issue with embedding")
        # Build document
        doc = {
            "path": path,
            "method": method.upper(),
